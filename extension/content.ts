(() => {
  ...
  function getStorage(): Promise<SavedMedia[]> {
    return new Promise((resolve) => {
      chrome.storage.local.get(STORAGE_KEY, (result) => {
        resolve((result[STORAGE_KEY] as SavedMedia[]) || []);
      });
    });
  }

  function setStorage(items: SavedMedia[]): Promise<void> {
    return new Promise((resolve) => {
  ...
  function extractMediaFromTweet(article: HTMLElement): { url: string; preview: string; type: 'image' | 'gif' }[] {
    const media: { url: string; preview: string; type: 'image' | 'gif' }[] = [];
  ...
  function extractAuthor(article: HTMLElement): { handle: string; name: string } {
  ...
  function createSaveButton(article: HTMLElement, mediaItems: { url: string; preview: string; type: 'image' | 'gif' }[]): HTMLDivElement {
