(() => {
  ...
  let allItems: _SavedMedia[] = [];
  let activeFilter: 'all' | 'gif' | 'image' = 'all';
  let searchQuery = '';

  function getStorage(): Promise<_SavedMedia[]> {
    return new Promise((resolve) => {
      chrome.storage.local.get(STORAGE_KEY, (result) => {
        resolve((result[STORAGE_KEY] as _SavedMedia[]) || []);
      });
    });
  }

  function setStorage(items: _SavedMedia[]): Promise<void> {
    return new Promise((resolve) => {
  ...
  function getFilteredItems(): _SavedMedia[] {
    let items = [...allItems];
    ...
    if (searchQuery.trim()) {
      ...
      items = items.filter(
        (item) =>
          item.tweetText.toLowerCase().includes(q) ||
  ...
  function initFilters(): void {
    ...
    filterBtns.forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        activeFilter = (btn as HTMLElement).dataset.filter as 'all' | 'gif' | 'image';
        renderGallery();
      });
    });
  }
